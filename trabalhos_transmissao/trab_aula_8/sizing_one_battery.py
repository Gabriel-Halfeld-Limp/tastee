...
bat1_b6 = Battery(id=9999, bus=b6.buses[0], p_max_mw=10, p_min_mw=-10, capacity_mwh=100000, soc_mwh=50000, cost_charge_mw=-1, cost_discharge_mw=350)
bat1_b118 = Battery(id=9999, bus=b118.buses[23], p_max_mw=10, p_min_mw=-10, capacity_mwh=100000, soc_mwh=50000, cost_charge_mw=-1, cost_discharge_mw=350)
...
def investiment_cost(net: Network):
    ...
    for battery in net.batteries:
        ...
        total_cost += battery.capacity_mwh * 500
    return total_cost

nets = [b6, b118]
hours = 5
battery_scaling = np.linspace(0, 1, 5)
...
for net in nets:
    ...
    for scale_b1 in battery_scaling:
        ...
        for h in range(hours):
            ...
            print(f"      -> Scale B1: {scale_b1:.4f} => Custo Operativo Hora {h}: ${hourly_cost:,.2f}")
        all_daily_costs.append(daily_cost)
        print(f"Scale B1: {scale_b1:.4f} => Custo Diário Total: ${daily_cost:,.2f}")
...
for net in nets:
    print(f"\n=== Resultados para a rede {net.name} ===")
    print(f"Melhor escala para Bateria 1: {best_results[net.name]['melhor_scale_b1']:.4f}")
    print(f"Custo Diário Mínimo: ${best_results[net.name]['min_daily_cost']:,.2f}")