...
class B6L8Charged(Network):
    ...
    def _create_generators(self):
        ...
        WindGenerator(id=4, bus=self.buses[0], p_max_mw=70, inverter_s_max_mva=50)
        WindGenerator(id=5, bus=self.buses[2], p_max_mw=50, inverter_s_max_mva=40)
        WindGenerator(id=6, bus=self.buses[3], p_max_mw=40, inverter_s_max_mva=30)
    
    def _create_loads(self):
        Load(id=1, bus=self.buses[1], p_mw=1*20.0, q_mvar= 8.5, cost_shed_mw=400)
        Load(id=2, bus=self.buses[2], p_mw=1*40.0, q_mvar=17.0, cost_shed_mw=400)
        Load(id=3, bus=self.buses[3], p_mw=1*30.0, q_mvar= 4.0, cost_shed_mw=400)
        Load(id=4, bus=self.buses[4], p_mw=1*30.0, q_mvar=12.7, cost_shed_mw=400)
        Load(id=5, bus=self.buses[5], p_mw=1*40.0, q_mvar=17.3, cost_shed_mw=400)